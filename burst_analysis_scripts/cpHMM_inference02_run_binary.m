...
writePath = [readPath dt filesep 'cpHMM_results' filesep];
mkdir(writePath)
...
ap_bins = data_ref.ap_bounds;%[-0.02 0.02];
ap_axis = ap_bins(1:end-1) + diff(ap_bins)/2;
knirps_thresholds = [0 4 6 12];
nBins = length(knirps_thresholds)-1;
...
% calculate knirps bins for each group
...
%     p_id = project_ap_zero_array(i,3);
    ...
    trace_filter = find(ap_group_vec==ap_id & (~zero_flag_vec | z_id));
       
    % get quantiles
%     p_vec = linspace(0,1,nBins);
    kni_vec = inference_data.mean_knirps_vec(trace_filter);
%     quant_vec = quantile(kni_vec,p_vec);
    kni_groups = discretize(kni_vec,knirps_thresholds);
    ...
    inference_struct(i).group_dp_vec = NaN(1,nBins);
    for n = 1:nBins
        ...
        % get count of total unique data points to ensure that we are not
        % oversampling 
        trace_i_list = trace_filter(kni_groups==n);
        t_vec = [];
        pid_vec = [];
        for m = 1:length(trace_i_list)
            t_temp = inference_data.time_vec_cell{trace_i_list(m)};
            t_vec = [t_vec t_temp];
            pid_vec = [pid_vec repelem(inference_data.particle_id_vec(trace_i_list(m)),length(t_temp))];
        end
        tp_array = [t_vec' pid_vec'];
        tpu = unique(tp_array,'rows');
        inference_struct(i).group_dp_vec(n) = size(tpu,1);
...
%     inference_struct(i).p_id = p_id;
    ...
    inference_struct(i).knirps_bins = knirps_thresholds;
    inference_struct(i).ap_pos = ap_axis(ap_id);
end    

save([readPath dt filesep 'inference_info.mat'],'inference_struct')
...
for i = 1:length(inference_struct)
    ...
    for k = 1:length(knirps_group_vec)
        ...
        for b = 1:nBoots
            ...
            sampleSize = min([inference_struct(i).group_dp_vec(group_i) sampleSize]);
