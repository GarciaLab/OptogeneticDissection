...
scatter(HIGH.time_vec,HIGH.knirps_vec_mean,70,'^','MarkerFaceColor',color_high,'MarkerEdgeColor','k')
...
Y = [WT.fluo_vec_mean(filter_WT)' HIGH.fluo_vec_mean(filter_HIGH)' LOW.fluo_vec_mean(filter_LOW)']/fluo_max;

% fit hill function
objective_fun = @(x) x(1)^x(2) ./ (x(1)^x(2) + X.^x(2)) - Y;

options = optimoptions('lsqnonlin','Display','off');
[x,~,residual,~,~,~,jacobian] = lsqnonlin(objective_fun,[2E5 5],[1E5 1],[Inf Inf],options);
...
errorbar(HIGH.knirps_vec_mean(filter_HIGH),HIGH.fluo_vec_mean(filter_HIGH),HIGH.knirps_vec_ste(filter_HIGH),HIGH.knirps_vec_ste(filter_HIGH),HIGH.fluo_vec_ste(filter_HIGH),HIGH.fluo_vec_ste(filter_HIGH),'o','Color',color_wt,'CapSize',0)
scatter(HIGH.knirps_vec_mean(filter_HIGH),HIGH.fluo_vec_mean(filter_HIGH),60,'^','MarkerFaceColor',color_high,'MarkerEdgeColor','k')
errorbar(LOW.knirps_vec_mean(filter_LOW),LOW.fluo_vec_mean(filter_LOW),knirps_vec_ste(filter_LOW),knirps_vec_ste(filter_LOW),fluo_vec_ste(filter_LOW),fluo_vec_ste(filter_LOW),'o','Color',color_wt,'CapSize',0)
scatter(LOW.knirps_vec_mean(filter_LOW),LOW.fluo_vec_mean(filter_LOW),60,'d','MarkerFaceColor',color_low,'MarkerEdgeColor','k')
errorbar(WT.knirps_vec_mean(filter_WT),WT.fluo_vec_mean(filter_WT),WT.knirps_vec_ste(filter_WT),WT.knirps_vec_ste(filter_WT),WT.fluo_vec_ste(filter_WT),WT.fluo_vec_ste(filter_WT),'o','Color',color_wt,'CapSize',0)
scatter(WT.knirps_vec_mean(filter_WT),WT.fluo_vec_mean(filter_WT),60,'MarkerFaceColor',color_wt,'MarkerEdgeColor','k')
...
%% plot wildtype input-output functino

fluo_max = 1.7E5;
xRange = linspace(0,15E5,1000);

filter_WT = (WT.time_vec>=6) & (WT.time_vec<=30);


X = WT.knirps_vec_mean(filter_WT)';
Y = WT.fluo_vec_mean(filter_WT)'/fluo_max;

% fit hill function
objective_fun = @(x) x(1)^x(2) ./ (x(1)^x(2) + X.^x(2)) - Y;

options = optimoptions('lsqnonlin','Display','off');
[x,~,residual,~,~,~,jacobian] = lsqnonlin(objective_fun,[5E5 5],[2E5 1],[9E5 20],options);
CI = nlparci(x,residual,'jacobian',jacobian);
...
input_output_wt_fig = figure;
hold on

errorbar(WT.knirps_vec_mean(filter_WT),WT.fluo_vec_mean(filter_WT),WT.knirps_vec_ste(filter_WT),WT.knirps_vec_ste(filter_WT),WT.fluo_vec_ste(filter_WT),WT.fluo_vec_ste(filter_WT),'o','Color',color_wt,'CapSize',0)
scatter(WT.knirps_vec_mean(filter_WT),WT.fluo_vec_mean(filter_WT),100,'MarkerFaceColor',color_wt,'MarkerEdgeColor','k')
...
random_sample_traces_HIGH = spot_traces_HIGH(randperm(size(spot_traces_HIGH, 1)), :);
random_sample_traces_LOW = spot_traces_LOW(randperm(size(spot_traces_LOW, 1)), :);

single_traces_WT_fig = figure;
...
single_traces_HIGH_fig = figure;
imagesc('XData',time_vec,'CData',random_sample_traces_HIGH(1:sample_num,:))
xlabel('time (min)')
xlim([0 30])
ylim([1 sample_num])
colorbar
...
nexttile
hold on
%plot(time_vec,spot_traces_HIGH(sample_HIGH,:),'- .')
plot(time_vec,spot_traces_HIGH(sample_HIGH,:),'-k','LineWidth',1)
scatter(time_vec,spot_traces_HIGH(sample_HIGH,:),30,'MarkerFaceColor',color_wt,'MarkerEdgeColor','k')
xlim([14.5 30])
ylim([-0.25E5 5.75E5 ])
pbaspect([3 1 1])
nexttile
...
plot(time_vec,spot_traces_LOW(sample_LOW,:),'-k','LineWidth',1)
scatter(time_vec,spot_traces_LOW(sample_LOW,:),30,'MarkerFaceColor',color_wt,'MarkerEdgeColor','k')
xlim([14.5 30])
ylim([-0.25E5 5.75E5 ])
pbaspect([3 1 1])
nexttile
...
plot(time_vec,spot_traces_WT(sample_WT,:),'-k','LineWidth',1)
scatter(time_vec,spot_traces_WT(sample_WT,:),30,'MarkerFaceColor',color_wt,'MarkerEdgeColor','k')
xlim([14.5 30])
ylim([-0.25E5 5.75E5 ])
pbaspect([3 1 1])