function io_ref_ra = build_ra_ref_set(projectNameRA, dataRoot, data_ref)

% Script to build io reactivation dataset
...
knirps_cal_intercept = data_ref.knirps_cal_intercept; %NL: dividng everything through by 1e5 for simplicity

% RA-specific parameters
time_bounds = [8 25];
window_size = 40; % size of lead and lag interval, in frames
sets_to_use = [1 2 3 4];
on_frames = [36 43 45 43]; % indicates timing of perturbation in each set
off_frame_ref = -1; % for inclusion, we only require that traces be off for the frame right before Knirps release
...
for s = 1:length(sets_to_use)
    ...
    set_id_vec_filt = set_id_vec(n_filter & ap_filter & time_filter);
    ...
    for i = 1:length(spot_struct_trunc)
        ...
        if max(diff(time_vec_raw)) < 2 && all(~isnan(knirps_vec))% mean(~isnan(knirps_vec)) > 0.9
    ...
    io_ref_struct_temp.knirps_array_norm(window_size+1:end,:) = ...
      (io_ref_struct_temp.knirps_array_norm(window_size+1:end,:)-knirps_cal_intercept)/knirps_cal_slope;
    ...
    io_ref_struct_temp.set_id_vec = set_id_vec_filt;
...
% io_ref_ra.set_id_vec = floor(io_ref_ra.particle_id_vec);
...
io_ref_ra.set_id_vec = io_ref_ra.set_id_vec(~nan_flags);
io_ref_ra.set_index = unique(io_ref_ra.set_id_vec);
...
for n = 1:nBoots
    ...
    for s = 1:length(boot_sets)
        boot_indices = [boot_indices find(io_ref_ra.set_id_vec==boot_sets(s))];
    end
