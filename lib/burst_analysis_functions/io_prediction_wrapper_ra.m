function sweepResults = io_prediction_wrapper_ra(sweepInfo,sweepResults)
    ...
    sweepInfo.HC = sweepResults.param_val_vec(strcmp(paramList,'HC_kon'));
    sweepInfo.KD = sweepResults.param_val_vec(strcmp(paramList,'KD_kon'));    
    sweepInfo.kon = sweepResults.param_val_vec(strcmp(paramList,'kon0'));
    sweepInfo.koff = sweepResults.param_val_vec(strcmp(paramList,'koff0'));
    sweepInfo.koff_slope = sweepResults.param_val_vec(strcmp(paramList,'HC_koff'));
    sweepInfo.r2 = sweepResults.param_val_vec(strcmp(paramList,'r2'));
    ...
    s_indices = randsample(1:size(sweepInfo.knirps_array_ra,2),n_traces,true);
...
%     delta_cdf_full = ra_cdf_full_mean-sweepInfo.reactivation_cdf;
%     ra_cdf_ste = sweepInfo.reactivation_cdf_ste;
%     logL_ra_cdf = (delta_cdf./2/ra_cdf_ste).^2 - log(2*pi*ra_cdf_ste.^2);
%     sweepResults.ra_fit = mean(logL_ra_cdf);
    
    % calculate simple R2 metrics
%     delta_cdf = delta_cdf ./ mean(sweepInfo.reactivation_cdf);
%     sweepResults.ra_fit = sum(delta_cdf.^2);
