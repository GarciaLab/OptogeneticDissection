...
function temp_gillespie = synthetic_rate_gillespie_io_v5(sweepInfo,tf_profile_array)
...
KD_kon = sweepInfo.KD;
HC_kon = sweepInfo.HC;
% KC_koff = sweepInfo.koff;
HC_koff = sweepInfo.HC_koff;
KD_koff = sweepInfo.KD_koff;
...
kon_curve_raw = RateMatrix(2,1) * tf_profile_array.^HC_kon ./ (KD_kon.^HC_kon +  tf_profile_array.^HC_kon);
kon_curve_in = NaN(size(kon_curve_raw));
kon_curve_out = interp1(t_ref_in,kon_curve_raw,t_ref_out);

% calculate linear I/O curve
koff_curve_raw = RateMatrix(1,2) * tf_profile_array.^HC_koff ./ (KD_koff.^HC_koff +  tf_profile_array.^HC_koff);
% lin_curve_in(lin_curve_in<rate_min) = rate_min;
koff_curve_in = NaN(size(kon_curve_in));
koff_curve_out = interp1(t_ref_in,koff_curve_raw,t_ref_out);
...
for t = 1:length(t_ref_out)
    ...
    R_array_temp(1,2,:) = koff_curve_out(t,1,:);    
    R_array_temp(2,1,:) = kon_curve_out(t,1,:);       
...
temp_gillespie.io_ref_in = kon_curve_in;
...
temp_gillespie.io_ref_out = kon_curve_out;
