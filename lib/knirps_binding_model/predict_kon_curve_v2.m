function [kon_curve, n_bound_curve] = predict_kon_curve_v2(repressorCVec,kon0,eOmega,KD,n_bs,n_calc_points)
...
for eb = 1:length(repressorCVec)  
  eb_eff = log(KD/repressorCVec(eb));
...
%   P_ind_array(:,eb) = state_probs_ind;
  ...
  % cooperativity
  state_probs_coop = stateProb_fun(eb_eff,-log(eOmega));    
...
kon_vec = kon0*(1-n_bound_vec'/n_bs);%.*exp(-n_bound_vec'*eInteraction);
